...
type MerchantQuery struct {
	...
	withParent       *MerchantQuery
	withChildren     *MerchantQuery
	withMerchantMeta *MerchantMetaQuery
...
func (mq *MerchantQuery) QueryParent() *MerchantQuery {
	query := (&MerchantClient{config: mq.config}).Query()
...
func (mq *MerchantQuery) QueryChildren() *MerchantQuery {
	query := (&MerchantClient{config: mq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := mq.prepareQuery(ctx); err != nil {
			return nil, err
...
// QueryMerchantMeta chains the current query on the "merchant_meta" edge.
func (mq *MerchantQuery) QueryMerchantMeta() *MerchantMetaQuery {
	query := (&MerchantMetaClient{config: mq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := mq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (mq *MerchantQuery) Clone() *MerchantQuery {
	...
	return &MerchantQuery{
		...
		withMerchantMeta: mq.withMerchantMeta.Clone(),
...
func (mq *MerchantQuery) WithParent(opts ...func(*MerchantQuery)) *MerchantQuery {
	query := (&MerchantClient{config: mq.config}).Query()
...
func (mq *MerchantQuery) WithChildren(opts ...func(*MerchantQuery)) *MerchantQuery {
	query := (&MerchantClient{config: mq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
// WithMerchantMeta tells the query-builder to eager-load the nodes that are connected to
// the "merchant_meta" edge. The optional arguments are used to configure the query builder of the edge.
func (mq *MerchantQuery) WithMerchantMeta(opts ...func(*MerchantMetaQuery)) *MerchantQuery {
	query := (&MerchantMetaClient{config: mq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	mq.withMerchantMeta = query
	return mq
}
...
func (mq *MerchantQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Merchant, error) {
	...
	if query := mq.withParent; query != nil {
		if err := mq.loadParent(ctx, query, nodes, nil,
			func(n *Merchant, e *Merchant) { n.Edges.Parent = e }); err != nil {
			return nil, err
		}
	}
	if query := mq.withChildren; query != nil {
		if err := mq.loadChildren(ctx, query, nodes,
			func(n *Merchant) { n.Edges.Children = []*Merchant{} },
			func(n *Merchant, e *Merchant) { n.Edges.Children = append(n.Edges.Children, e) }); err != nil {
			return nil, err
		}
	}
	if query := mq.withMerchantMeta; query != nil {
		if err := mq.loadMerchantMeta(ctx, query, nodes,
			func(n *Merchant) { n.Edges.MerchantMeta = []*MerchantMeta{} },
			func(n *Merchant, e *MerchantMeta) { n.Edges.MerchantMeta = append(n.Edges.MerchantMeta, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (mq *MerchantQuery) loadParent(ctx context.Context, query *MerchantQuery, nodes []*Merchant, init func(*Merchant), assign func(*Merchant, *Merchant)) error {
...
func (mq *MerchantQuery) loadChildren(ctx context.Context, query *MerchantQuery, nodes []*Merchant, init func(*Merchant), assign func(*Merchant, *Merchant)) error {
	...
	query.Where(predicate.Merchant(func(s *sql.Selector) {
		s.Where(sql.InValues(merchant.ChildrenColumn, fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (mq *MerchantQuery) loadMerchantMeta(ctx context.Context, query *MerchantMetaQuery, nodes []*Merchant, init func(*Merchant), assign func(*Merchant, *MerchantMeta)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uint64]*Merchant)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.MerchantMeta(func(s *sql.Selector) {
		s.Where(sql.InValues(merchant.MerchantMetaColumn, fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "merchant_id" returned %v for node %v`, fk, n.ID)
		}
		assign(node, n)
	}
	return nil
