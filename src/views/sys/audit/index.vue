<template>
  <div>
    ...
    <AuditDrawer @register="registerDrawer" @success="handleSuccess" />
  </div>
</template>
<script lang="ts">
  ...
  export default defineComponent({
    ...
    components: { BasicTable, AuditDrawer, TableAction, Button, DeleteOutlined },
    setup() {
      const { t } = useI18n();
      ...
      async function handleDelete(record: Recordable) {
        const result = await deleteAudit({ ids: [record.id] }, 'modal');
        if (result.code === 0) {
